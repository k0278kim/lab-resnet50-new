...
def fit_one_epoch(net, softmaxloss, epoch, epoch_size, epoch_size_val, gen, gen_test, Epoch, cuda):
    ...
    # ---------- Test 1 (same TRAIN set, two modes) ----------
    net.train()
    ...
    print(f"[Test1] Train-set acc with net.eval():  {acc_eval_mode:.2f}%")
    # --------------------------------------------------------
...
if __name__ == '__main__':
    ...
    Batch_size = 512
    Init_Epoch = 0
    Fin_Epoch = 5     #Epoch : originally 100
    ...
    gen_test = DataLoader(dataset=test_dataset, batch_size=Batch_size // 2, shuffle=False, num_workers=0)
